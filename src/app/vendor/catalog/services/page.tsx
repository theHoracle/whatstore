...
import {  useStoreServices } from "@/hooks/use-vendor-stores";
...
export default function StoreservicesPage() {
 ...
 const activeStoreId = vendorState?.activeStoreId as number;
  const { data: services, isLoading, error } = useStoreServices(activeStoreId);
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Store services</h1>
          <p className="text-muted-foreground">Manage services for this store</p>
        </div>
        <Link 
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input 
            type="text" 
            placeholder="Search store services..." 
